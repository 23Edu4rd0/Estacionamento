...
async def get_all_designations(session: AsyncSession):
    result = await session.scalars(select(DesignationModel))
    return result.all()


async def get_designations_by_date(session: AsyncSession, designation_date: str):
    result = await session.scalars(
        select(DesignationModel).where(DesignationModel.event_date == designation_date)
    )
    return result.all()
...
async def edit_designation_repo(
    session: AsyncSession, designation_id: int, updated_data
):
